...
void list(char *dir)
{
	...
	while ((dirptr = readdir(cwd)) != NULL)
	{ // Iterate over opened directory and print contents
		if (dirptr->d_name[0] != '.')
		{
			printf("\t%s\n", dirptr->d_name);
		}
...
int main()
{
	...
	while (fgets(input, 1024, stdin))
	{
		...
		for (j = 0, ptr = strtok(tmp, " "); ptr; ptr = strtok(NULL, " "), j++)
		{ // Take the user input and put each word in an array
